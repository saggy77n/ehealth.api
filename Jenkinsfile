      ...
      steps {
        container(name: 'gcloud', shell: '/bin/sh') {
          withCredentials([file(credentialsId: 'e7e3e6df-8ef5-4738-a4d5-f56bb02a8bb2', variable: 'KEYFILE')]) {
            ...
            for ((i = 0; i < 10; i++))
              do 
                gcloud container node-pools create ehealth-build-${env.BUILD_NUMBER} --cluster=dev --machine-type=n1-highcpu-16 --node-taints=ci=${env.BUILD_TAG}:NoSchedule --node-labels=node=${env.BUILD_TAG} --num-nodes=1 --zone=europe-west1-d --preemptible
                  if [[ $? -ne 0 ]]; then
                  sleep 25
                  continue
                  fi
